...
sudo apt update
pkgs='curl uidmap apt-transport-https ca-certificates gnupg lsb-release docker.io docker-compose ffmpeg libopencv-dev libopencv*'
...
if ! dpkg -s $pkgs >/dev/null 2>&1; then
  ...
  sudo ln -s /opt/DECK/libffmpeg.so /usr/lib/libffmpeg.so
